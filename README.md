...
## Важно о SerpAPI
Был выбран провайдер ThorData, так как имеет высокие беслпатные лимиты. Доступ к SerpAPI из РФ затруднителен через любого провайдера, поэтому может потребоваться VPN (необязателен, зависит от интернет-провайдера).
